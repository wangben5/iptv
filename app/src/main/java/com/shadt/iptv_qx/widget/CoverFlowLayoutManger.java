...
public class CoverFlowLayoutManger extends RecyclerView.LayoutManager {
    ...
    public CoverFlowLayoutManger(boolean isFlat, boolean isGreyItem,
        ...
        if (cstInterval >= 0) {
//            mIntervalRatio = cstInterval;   这是以前的
    ...
    /**
     * 布局Item
     * <p>注意：record1，先清除已经超出屏幕的item
     * <p>     record2，再绘制可以显示在屏幕里面的item
     */
    ...
    private void layoutItem(View child, Rect frame) {
        ...
        if (!mIsFlatFlow) { //不是平面普通滚动的情况下才进行缩放
            child.setScaleX(1.5f*(computeScale(frame.left - mOffsetAll))); //缩放
            child.setScaleY(1.2f*(computeScale(frame.left - mOffsetAll))); //缩放
        }
...
//            cm.setSaturation(0.9f);
    ...
    private float getIntervalDistance() {
        return mDecoratedChildWidth * mIntervalRatio+100;
    }
